...
```{r}
query = '"pharmacogenetics"[MeSH Terms] OR "pharmacogenetics"[All Fields] OR "pharmacogenomics"[All Fields]'
data <- RetrieveArticleData(query,1980,2018,50000)
article_metadata <- RetrieveMostArticleMetadata(data)
article_most_metadata <- RetrieveMostArticleMetadata(data)
```
...
Need to get affiliation data out

```{r}
affiliations <- data.frame(unlist(Affiliation(data)))
write.csv(affiliations, file = "pharmcogenomicsAffiliations.csv")

```

Journal titles top 5 graph
```{r}
jTitlesTop <- JournalTitlesFreq(article_most_metadata)
write.csv(jTitlesTop, file = "pharmcogenomicsJTitles.csv")
...
library(plyr)
MeSH <- GetMeSH(data)
MeSHCount <- CountMeSH(MeSH)
write.csv(MeSHCount, file = "pharmcogenomicsMeSHCount.csv")
...
```{r}
MapPubsPerCountry(data,article_metadata)
```
